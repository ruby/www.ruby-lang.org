...
를 배우는 동안 보게 될 의 주요 기능들의 요점과 힌트들을 알아봅시다.
...
많은  뉴비들은 심볼을 이해하는데 어려워하고, 어디에 사용하는 것인지
궁금해 합니다.
...
는 몇몇 이름 규칙을 강제합니다. 대문자로 시작하는 식별자는 상수입니다.
...
이 구문은 “0 is false”를 출력합니다. 같은 구문은 에서:
...
는 약간 다릅니다. `public`은 말 그대로 공개입니다. `private`은 메서드에서만
...
사용할 수 있습니다. [ FAQ][faq]에서 가져온 예제입니다.
...
에서는, 메서드 이름이 물음표나 느낌표로 끝날 수 있습니다.
...
는 특정 메시지에 응답할 메서드를 찾을 수 없을 때에도 포기하지
...
의 대부분의 연산자는 메서드 호출의 (몇가지 우선순위 규칙을
...
더 많은 에 관한 정보를 보고 싶으시면 [문서](/ko/documentation) 항목을
보세요.
