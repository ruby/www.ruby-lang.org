...
### 할당하면 객체의 새 복사본이 생성되나요?
{: #assignment}

모든 변수와 상수는 어떤 객체를 참조(가리킴)합니다. (초기화되지 않은 로컬 변수는
예외입니다. 이 경우 아무것도 참조하지 않습니다. 사용하면 `NameError` 예외가
발생합니다) 변수에 할당하거나 상수를 초기화할 때 변수 또는 상수가 참조하는
객체를 설정합니다.

따라서 할당 자체만으로는 객체의 새 복사본이 생성되지 않습니다.

특수한 경우에 대한 더 자세한 설명을 하겠습니다. `Fixnum`, `NilClass`,
`TrueClass`, `FalseClass`의 인스턴스는 변수나 상수에 직접 포함되며 참조가 포함
되지 않습니다. 숫자 `42` 또는 상수 `true`를 보유한 변수는 실제로 값을 보유
하는 것이지 참조를 보유하는 것이 아닙니다. 따라서 대입은 물리적으로 이러한
...
(주의: 클래스 정의는 실행 코드이므로 클래스가 정의에 포함된 추적 메시지가
기록됩니다.)

블록(`{ ... }` 나 `do ... end`)은 거의 새로운 스코프를 도입합니다 ;-)
블록 내에서 생성된 로컬 변수는 블록 외부에서 액세스할 수 없습니다.
그러나 블록 내의 로컬 변수가 호출자 스코프의 기존 로컬 변수와 이름이 같으면 새
로컬 변수가 생성되지 않으며, 이후 블록 외부에서 해당 변수에 액세스할 수 있습
니다.
...
이는 스레딩을 사용할 때 중요해지는데, 각 스레드는 스레드 블록에 로컬로 있
는 변수의 자체 복사본을 받습니다.
...
다음 결과가 나올 수 있습니다. (스케줄러가 `Thread.pass`에 의해 힌트된 대로
...
`while`, `until`, `for`는 블록이 아닌 제어 구조이므로 그 안에 있는 로컬
 변수는 둘러싸는 환경에서 액세스할 수 있습니다. 그러나 `loop`는 메서드
이며 관련 블록은 새로운 스코프를 도입합니다.

### 로컬 변수는 언제 액세스할 수 있나요?
...
어떤 경우인지 판단하기 위해 Ruby는 할당 문을 찾습니다.
소스에서 `a`를 사용하기 전 어느 지점에서 `a`가 할당되는 것을 확인하면 `a`를
...
구문 분석 중에 Ruby는 첫 번째 `puts` 문에서 `a`가 사용된 것을 보고 `a`에
 대한 할당을 아직 보지 못했기 때문에 메서드 호출이라고 가정합니다. 하지만 두
번째 `puts` 문에 도달하면 대입이 확인되었으므로 `a`를 변수로 취급합니다.


주의: 할당은 실행될 필요가 없으며 확인만 하면됩니다. 이 프로그램은 오류를
발생시키지 않습니다:
...
하면 이후에 루프에 나타나는 로컬 변수에 대한 액세스 시간이 빨라지는 추가적인
이점이 있습니다.
...
실제 인수는 메서드가 호출될 때 형식 인수에 할당됩니다.
(할당의 의미에 대한 자세한 내용은 [assignment](#assignment)를 참조하세요.)
...
객체 참조를 전달할 때 메서드가 전달된 변경 가능한 객체의 내용을 수정할 수
있습니다.
...
### 형식 인수에 대한 할당이 실제 인수에 영향을 주나요?

형식 인수는 지역 변수입니다. 메서드 내에서 형식 인수를 할당하면 단순히 인수가
다른 객체를 참조하도록 변경됩니다.

### 형식 인수를 통해 메서드를 호출하면 어떻게 되나요?

모든 Ruby 변수(메서드 인수 포함)는 객체에 대한 참조로 작동합니다. 이러한 객체
에서 메서드를 호출하여 객체의 상태를 가져오거나 변경하고 객체가 어떤 작업을
 수행하도록 할 수 있습니다. 메서드에 전달된 객체로 이 작업을 수행할 수 있습
니다. 이러한 종류의 부작용으로 인해 프로그램을 따라가기 어려울 수 있으
므로 이 작업을 수행할 때 주의해야 합니다.

### 인수 앞에 `*`가 붙으면 무슨 뜻인가요?

별표를 공식 매개변수 목록의 일부로 사용하면 인수를 배열로 수집하고 별표가
...
이러한 매개변수는 실제로 로컬 변수입니다. 블록이 실행될 때 같은 이름의 기존
로컬 변수가 존재하면 블록을 호출하면 해당 변수가 수정됩니다. 이는 좋을 수도
나쁠 수도 있습니다.

일반적으로 인자는 `yield`(또는 `yield`를 호출하는 이터레이터)를 사용하거나
...
`b.concat("d")`를 호출하면 해당 객체에 대해 concat 메서드를 호출하여 `"abc"`
에서 `"abcd"`로 변경합니다. a`와 `A`도 동일한 객체를 참조하기 때문에 겉보기
값도 변경됩니다.

실제로는 생각보다 큰 문제가 되지 않습니다.
...
재할당할 수 없지만, 그 외에는 마음대로 변경할 수 있습니다.
상수에 새 값이 할당되면 경고가 표시됩니다.
...
`load` 및 `require`는 로컬 변수를 별도의 익명 네임스페이스에 저장하도록 정렬
하여 효과적으로 로컬 변수를 제거하기 때문에 오류가 발생합니다. 이는 코드가
