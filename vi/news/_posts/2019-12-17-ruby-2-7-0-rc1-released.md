---
layout: news_post
title: "Ruby 2.7.0-rc1 đã được phát hành"
author: "naruse"
translator:
...
Chúng tôi vui mừng thông báo phiên bản Ruby 2.7.0-rc1 đã được phát hành.

Một bản release candidate được phát hành để thu thập phản hồi cho bản phát hành
chính thức dự kiến vào tháng 12.
Bản rc1 này chủ yếu được phát hành để xác nhận tính tương thích của
tham số từ khóa.
...
hơn được sử dụng, và heap có thể thân thiện hơn với CoW.
...
LƯU Ý: Quá nhiều cảnh báo phản đối về sự không tương thích tham số từ khóa
đã được chỉ ra là quá dài dòng. Hiện tại, hai giải pháp khả thi
đang được thảo luận; tắt cảnh báo phản đối theo mặc định
([#16345](https://bugs.ruby-lang.org/issues/16345))
hoặc loại bỏ các cảnh báo trùng lặp
([#16289](https://bugs.ruby-lang.org/issues/16289)).
Quyết định cuối cùng chưa được đưa ra, nhưng sẽ được sửa trước bản phát hành chính thức.

## Các tính năng mới đáng chú ý khác

* Toán tử tham chiếu phương thức, `.:`, đã được giới thiệu
  như một tính năng thử nghiệm trong các bản preview trước, nhưng đã bị hoàn tác.
...
* Gọi phương thức private trên `self` giờ đây được cho phép.
...
* `lambda` không có block trong phương thức được gọi với block sẽ phát sinh lỗi.
...
* ~~`Regexp#match{?}` với `nil` phát sinh TypeError như String, Symbol.
  [[Feature #13083]](https://bugs.ruby-lang.org/issues/13083)~~ đã hoàn tác

Xem [NEWS](https://github.com/ruby/ruby/blob/v2_7_0_rc1/NEWS)
hoặc [nhật ký commit](https://github.com/ruby/ruby/compare/v2_6_0...v2_7_0_rc1)
để biết thêm chi tiết.

Với những thay đổi đó, [{{ release.stats.files_changed }} tệp đã thay đổi, {{ release.stats.insertions }} thêm vào(+), {{ release.stats.deletions }} xóa đi(-)](https://github.com/ruby/ruby/compare/v2_6_0...v2_7_0_rc1)
kể từ Ruby 2.6.0!
